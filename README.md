...
|:----------------:|:-----------------------------------------------------------------------------------------------------------------------------------------:|:-----------------------------------------------------:|:------------------------------------------------:|
| Object Detection |                               `object detection`  (detect object by providing category names like dog, cat)                               |   [detection_example.py](tutorials/detection_example/detection_example.py)   | [detection_example.ipynb](tutorials/detection_example/_full_notebook.ipynb) |
|                  |                         `object referring`  (detect object by providing descriptions like a blue dog on the roof)                         | [referring_example.py](tutorials/detection_example/referring_example.py)      |                                                  |
|                  | `gui grounding` (detect and locate user interface elements by providing descriptions (e.g. “the blue Submit button at the bottom right”). | [gui_grounding_example.py](tutorials/detection_example/gui_grounding_example.py)  |                                                  |
|                  |                    `layout grounding` (detect and elements in a layout by using categories like title, caption, figure)                   | [layout_grouding_examle.py](tutorials/detection_example/layout_grouding_examle.py) |                                                  |
| Object Pointing |                               `object pointing`  (pointing to object by providing category names like dog, cat)                               |   [object_pointing_example.py](tutorials/pointing_example/object_pointing_example.py)   | [object_pointing_example.ipynb](tutorials/pointing_example/_full_notebook.ipynb) |
|                  |                         `gui pointing`  (pointing to gui elements by providing descriptions like “the home button”)                         | [gui_pointing_example.py](tutorials/pointing_example/gui_pointing_example.py)      |                                                  |
|                  | `affordance pointing` (detect where interaction is possible by pointing to functional regions (e.g. “the handle of the mug”,)). | [affordance_pointing_example.py](tutorials/pointing_example/affordance_pointing_example.py)  |                                                  |
| visual prompting | `visual prompting` (detect object by interactively providing a box (e.g. draw a box around the dog to detect it). | [visual_prompt_example.py](tutorials/visual_prompting_example/visual_prompt_example.py) | [visual_prompt_example.ipynb](tutorials/visual_prompting_example/_full_tutorial.ipynb) |
| OCR | `ocr word box` (detect and recognize each word in the image by outputting boxes (e.g. box around “STOP” with text “STOP”)). | [ocr_word_box_example.py](tutorials/ocr_example/ocr_word_box_example.py) | [OCR.ipynb](tutorials/ocr_example/_full_tutorial.ipynb) |
|                  | `ocr textline box` (detect and recognize each line of text in the image by outputting boxes (e.g. box around “OPEN 24 HOURS”).). | [ocr_textline_box_example.py](tutorials/ocr_example/ocr_textline_box_example.py) | |
|                  | `ocr polygon` (detect and recognize text by outputting polygons (not just boxes) for each word/line/region). | [ocr_polygon_example.py](tutorials/ocr_example/ocr_polygon_example.py) |  |
| Keypointing | `person keypointing`: detect human body keypoints and output coordinates/skeleton per person (e.g. nose, shoulders, elbows, wrists, hips, knees, ankles). | [person_keypointing_example.py](tutorials/keypointing_example/person_keypointing_example.py) | [keypointing.ipynb](tutorials/keypointing_example/_full_tutorial.ipynb)|
|             | `animal keypointing`: detect keypoints on animals and output coordinates/skeleton per instance (e.g. nose, ears, paws, tail).                             |  [animal_keypointing_example.py](tutorials/keypointing_example/animal_keypointing_example.py)                                                |                                                  |